...
extern "C"
JNIEXPORT jint JNICALL
Java_com_coder_x264cmake_jni_X264Encode_encode_1data(JNIEnv *env, jobject thiz, jbyteArray data,
                                                     jint length) {
    int ret = 0;
    ...
    switch (x264_csp) {
        case X264_CSP_I444:
            ...
            memcpy(pic_in->img.plane[0], bytes, size);
            memcpy(pic_in->img.plane[1], bytes + size, size);
            memcpy(pic_in->img.plane[2], bytes + size * 2, size);
            break;
        case X264_CSP_I422:
            // 三个planar，y:1 u:1/2 v:1/2
            memcpy(pic_in->img.plane[0], bytes, size);
            memcpy(pic_in->img.plane[1], bytes + size, size / 2);
            memcpy(pic_in->img.plane[2], bytes + (size * 3 / 2), size / 2);
            break;
        case X264_CSP_I420:
            // 三个planar，y:1 u:1/4 v:1/4
            memcpy(pic_in->img.plane[0], bytes, size);
            memcpy(pic_in->img.plane[1], bytes + size, size / 4);
            memcpy(pic_in->img.plane[2], bytes + (size * 5 / 4), size / 4);
            break;
        case X264_CSP_NV21:
            // 只有两个planar，即是 y + vu
            memcpy(pic_in->img.plane[0], bytes, size);
